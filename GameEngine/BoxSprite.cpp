...
{
	this->privBoxSprite( spriteBatchName,  _spriteName, _lineWidth, _color, Rect(gSprite->posX, gSprite->posY, gSprite->origWidth, gSprite->origHeight) );
}
...
{
	this->privBoxSprite( spriteBatchName,  _spriteName, _lineWidth, _color, destRect );
}



void BoxSprite :: privBoxSprite(  SpriteBatchName::Name spriteBatchName, SpriteName::Name _spriteName, float _lineWidth, const Vect &_color, Rect destRect)
{
	this->setName( 0 );
...
void BoxSprite::transform( void )
{
	...
	Matrix TransToOriginLowerLeft( TRANS, -400, -300.f, 0.0f);
